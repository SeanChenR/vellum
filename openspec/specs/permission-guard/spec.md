...
HTTP-shape authorization gate for write-side canvas surfaces. Given `(session, canvasId, allowed-roles)`, the module collapses the five-step decision matrix (null-session / unknown-canvas / no-role / wrong-role / allowed-role) into a single discriminated result so route handlers translate one call into the correct 401 / 403 / 404 / ok response with the right i18n `errorKey`. The guard is the shared entry point future Phase 2 AI write endpoints (M13/M14) and the dev mutate endpoint (M12.1) call before any canvas mutation runs.
