...
export function DashboardPage() {
  ...
  function handleMoveCanvas(canvasId: string, targetFolderId: string | null) {
  ...
  return (
    <div className="flex min-h-screen bg-warm-50">
      ...
      <main className="flex-1 overflow-y-auto p-8">
        ...
        <section aria-labelledby="section-owned" className="mb-10">
          <div className="mb-4 flex items-center justify-between">
            <h2
              id="section-owned"
              className="text-xl font-semibold text-ink-navy"
            >
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {ownedList.canvases.map((canvas) => (
                <CanvasCard
                  ...
                  onMove={(c) =>
                    ownedList.moveCanvas.mutate({ id: c.id, folderId: null })
                  }
                />
              ))}
            </div>
        ...
        <section aria-labelledby="section-shared">
          <h2
            id="section-shared"
            className="mb-4 text-xl font-semibold text-ink-navy"
          >
      ...
      <CanvasRenameDialog
        ...
        currentTitle={
          dialog.kind === "canvas-rename" ? dialog.canvas.title : ""
        }
        onConfirm={(newTitle) => {
        ...
        onClose={() => setDialog({ kind: "none" })}
        isPending={
          dialog.kind === "canvas-rename" &&
          ownedList.renameCanvas.isPending
        }
      />
      ...
      <CanvasDeleteDialog
        ...
        canvasTitle={
          dialog.kind === "canvas-delete" ? dialog.canvas.title : ""
        }
        onConfirm={() => {
        ...
        onClose={() => setDialog({ kind: "none" })}
        isPending={
          dialog.kind === "canvas-delete" &&
          ownedList.deleteCanvas.isPending
        }
      />
      ...
      <FolderRenameDialog
        ...
        currentName={
          dialog.kind === "folder-rename" ? dialog.folder.name : ""
        }
        onConfirm={(newName) => {
        ...
        onClose={() => setDialog({ kind: "none" })}
        isPending={
          dialog.kind === "folder-rename" && renameFolder.isPending
        }
      />
      ...
      <FolderDeleteDialog
        ...
        folderName={
          dialog.kind === "folder-delete" ? dialog.folder.name : ""
        }
        onConfirm={handleFolderDeleteConfirm}
        onClose={() => setDialog({ kind: "none" })}
        isPending={
          dialog.kind === "folder-delete" && deleteFolder.isPending
        }
